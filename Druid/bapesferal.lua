...
Routine:RegisterRoutine(function()
    ...
    local function manacost(spellname)
        if not spellname then
            ...
            if costTable == nil then return 0 end
            for _, costInfo in pairs(costTable) do
                if costInfo.type == 0 then return costInfo.cost end
            end
        end
    end
    ...
    local function do_combat()
        ...
        -- Dire Bear
        if not bearForm then bearForm = highestrank(9634) end
        ...
        if healInCombat and not buff(frenziedRegeneration) then
            ...
            if health() <= healPercentage and castable(rejuvenation, player) and
                not buff(rejuvenation, player) then
                return cast(rejuvenation, player)
            end

            if health() <= healPercentage and castable(regrowth, player) and
                not buff(regrowth, player) then
    ...
    local function do_resting()
        ...
        -- MOTW Ranks
        local motwRanks = {
            21849, 21850, 26991, 1126, 5232, 8907, 9884, 9885, 26990
        }
        ...
        if healOutOfCombat then
            if health() <= healPercentage and castable(rejuvenation) and
                not buff(rejuvenation, player) then
                return cast(rejuvenation, player)
            end

            if health() <= healPercentage and castable(regrowth) and
                not buff(regrowth, player) then
                return cast(regrowth, player)
            end

            if health() <= healPercentage and castable(healingTouch) and
                not buff(healingTouch, player) then
        ...
        if castable(MarkOfTheWild, player) and not buff(MarkOfTheWild, player) and
            not buff(GiftOfTheWild, player) then
            for _, motwRank in pairs(motwRanks) do
                if buff(motwRank, player) then break end
            end
        ...
        if castable(Thorns, player) and not buff(Thorns, player) then
            for _, thornRank in pairs(thornsRanks) do
                if buff(thornRank, player) then break end
            end
        ...
        -- Cat Form
        if UnitExists(target) and alive(target) and not buff(catForm, player) and
            castable(catForm, player) and distance(player, target) <=
            math.random(25, 40) then return cast(catForm, player) end

        -- Stealth
        if wowex.wowexStorage.read("useStealth") and UnitExists(target) and
            alive(target) and not buff(stealth, player) and
            castable(stealth, player) and distance(player, target) <= 15 then
        ...
        -- Pounce
        if UnitExists(target) and spellInRange(pounce) and
            castable(pounce, target) then return cast(pounce, target) end
    end
...
local bapesFeralTable = {
    ...
    tabgroup = {
        {text = "Healing", value = "one"}, {text = "Buffs", value = "two"},
